...
namespace erl::geometry {
    ...
    public:
        struct Setting : public common::Yamlable<Setting> {
            ...
            explicit Setting(const int num_node_types, const int capacity_per_type = 1, const double min_squared_distance = 0.04)
    ...
    public:
        static std::shared_ptr<NodeContainerMultiTypes>
        ...
        [[nodiscard]] std::size_t
        Capacity(const int type) const override {
        ...
        [[nodiscard]] std::size_t
        Size(const int type) const override {
            ERL_DEBUG_ASSERT(type >= 0 && type < m_setting_->num_node_types, "Invalid node type.");
        ...
        Begin(const int type) override {
            ERL_DEBUG_ASSERT(type >= 0 && type < m_setting_->num_node_types, "Invalid node type.");
            return m_nodes_[type].begin();
        }

        [[nodiscard]] std::vector<std::shared_ptr<Node>>::const_iterator
        Begin(const int type) const override {
            ERL_DEBUG_ASSERT(type >= 0 && type < m_setting_->num_node_types, "Invalid node type.");
            return m_nodes_[type].begin();
        }

        std::vector<std::shared_ptr<Node>>::iterator
        End(const int type) override {
            ERL_DEBUG_ASSERT(type >= 0 && type < m_setting_->num_node_types, "Invalid node type.");
            return m_nodes_[type].end();
        }

        [[nodiscard]] std::vector<std::shared_ptr<Node>>::const_iterator
        End(const int type) const override {
            ERL_DEBUG_ASSERT(type >= 0 && type < m_setting_->num_node_types, "Invalid node type.");
            return m_nodes_[type].end();
        }
        ...
        void
        CollectNodesOfType(const int type, std::vector<std::shared_ptr<Node>> &out) const override {
            ERL_DEBUG_ASSERT(type >= 0 && type < m_setting_->num_node_types, "Invalid node type.");
        ...
        void
        CollectNodesOfTypeInAabb2D(const int type, const Aabb2D &area, std::vector<std::shared_ptr<Node>> &nodes) const override {
            ERL_DEBUG_ASSERT(type >= 0 && type < m_setting_->num_node_types, "Invalid node type.");
            auto &nodes_of_type = m_nodes_[type];
        ...
        void
        CollectNodesOfTypeInAabb3D(const int type, const Aabb3D &area, std::vector<std::shared_ptr<Node>> &nodes) const override {
            ERL_DEBUG_ASSERT(type >= 0 && type < m_setting_->num_node_types, "Invalid node type.");
            auto &nodes_of_type = m_nodes_[type];
            if (nodes_of_type.empty()) { return; }
            nodes.reserve(nodes.size() + nodes_of_type.size());
        ...
        Insert(const std::shared_ptr<Node> &node, bool &too_close) override {
            ...
            const auto &min_squared_distance = m_setting_->node_type_min_squared_distance[node_type];
            ...
            if (!nodes_of_type.empty()) {  // not empty
                too_close = std::any_of(nodes_of_type.begin(), nodes_of_type.end(), [&](const std::shared_ptr<Node> &n) {
                    return (n->position - node->position).squaredNorm() < min_squared_distance;
                });
                if (too_close) { return false; }
            }

            if (nodes_of_type.size() >= static_cast<std::size_t>(m_setting_->node_type_capacity[node_type])) { return false; }
        ...
        Remove(const std::shared_ptr<const geometry::Node> &node) override {
            ...
            const auto begin = nodes_of_type.begin();
    ...
    private:
        explicit NodeContainerMultiTypes(const std::shared_ptr<Setting> &setting)
            : m_setting_(setting) {
            Reset();
        }
...
template<>
struct YAML::convert<erl::geometry::NodeContainerMultiTypes::Setting> {
    static Node
    ...
    decode(const Node &node, erl::geometry::NodeContainerMultiTypes::Setting &setting) {
        ...
        ERL_ASSERTM(static_cast<int>(setting.node_type_capacity.size()) == setting.num_node_types, "node_type_capacity.size() != num_node_types");
        ERL_ASSERTM(
            static_cast<int>(setting.node_type_min_squared_distance.size()) == setting.num_node_types,
