...
GenerateTrajectory(const int n = 50, const int repeats = 1) {
    ...
    for (int i = 0; i < n; ++i) {
        Dtype pose_angle = 0;
        if (i > 0) { pose_angle = std::atan2(trajectory[i].y() - trajectory[i - 1].y(), trajectory[i].x() - trajectory[i - 1].x()); }
...
VisualizeResult(
    ...
    constexpr int s = 9;
    cv::resize(img_prob_occupied_rgb, img_prob_occupied_rgb, cv::Size(), s, s, cv::INTER_LINEAR_EXACT);
    cv::resize(img_gradient_norm_rgb, img_gradient_norm_rgb, cv::Size(), s, s, cv::INTER_LINEAR_EXACT);
    ...
    for (std::size_t idx = 0; idx < waypoints.size(); ++idx) {
        ...
        // cv::circle(img_prob_occupied_rgb, cv::Point(px[0], px[1]), 2, cv::Scalar(255, 255, 255), -1);
        ...
        for (long i = 0; i < points.cols(); ++i) {
            ...
            cv::circle(img_prob_occupied_rgb, cv::Point(px[0], px[1]), 1, cv::Scalar(0, 255, 0), -1);
        }
    }
...
template<typename Dtype>
void
TestBayesianHilbertMap2D(const int hinged_grid_size, const int test_grid_size, const Dtype rbf_gamma, const bool diagonal_sigma, const bool faster) {
    ...
    Eigen::Matrix2X<Dtype> hinged_points = GenerateGridPoints(hinged_grid_size, map_boundary).second;
    ...
    for (const auto &pose: trajectory) {
        auto scan = lidar.Scan(static_cast<double>(pose[2]), pose.template head<2>().template cast<double>(), true);
        const Eigen::Matrix2<Dtype> rotation = Eigen::Rotation2D<Dtype>(pose[2]).toRotationMatrix();
        Eigen::Matrix2X<Dtype> points(2, scan.size());
        for (long i = 0; i < scan.size(); ++i) { points.col(i) << scan[i] * (rotation * ray_dirs_frame.col(i).cast<Dtype>()) + pose.template head<2>(); }
        scanned_points.push_back(points);
        waypoints.push_back(pose.template head<2>());

        // bhm.Update(pose.template head<2>(), points, num_points, dataset_points, dataset_labels, hit_indices);
        bhm.GenerateDataset(pose.template head<2>(), points, num_points, dataset_points, dataset_labels, hit_indices);
        ...
        // bhm.RunExpectationMaximization(dataset_points, dataset_labels, num_points);
        bhm.PrepareExpectationMaximization(dataset_points, num_points);
        for (int itr = 0; itr < bhm_setting->num_em_iterations; ++itr) {
            bhm.RunExpectationMaximizationIteration(dataset_labels, num_points);
            // predict and visualize
            constexpr bool with_sigmoid = false;
            constexpr bool parallel = true;
            bhm.Predict(test_points, faster, true, with_sigmoid, parallel, predicted_prob_occupied, predicted_gradient);
            bhm.PredictGradient(surf_points, faster, with_sigmoid, parallel, predicted_gradient_surf);
            VisualizeResult(test_grid, waypoints, scanned_points, predicted_prob_occupied, predicted_gradient, surf_points, predicted_gradient_surf);
        }
    }
...
TEST(BayesianHilbertMap, 2Dd) {
    TestBayesianHilbertMap2D<double>(g_options.hinged_grid_size, g_options.test_grid_size, g_options.rbf_gamma, g_options.diagonal_sigma, g_options.faster);
}

TEST(BayesianHilbertMap, 2Df) {
    TestBayesianHilbertMap2D<float>(g_options.hinged_grid_size, g_options.test_grid_size, g_options.rbf_gamma, g_options.diagonal_sigma, g_options.faster);
}
