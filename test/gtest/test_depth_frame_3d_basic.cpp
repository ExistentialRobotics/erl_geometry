...
TEST(DepthFrame3D, Basic) {
    ...
    for (int i = 0; i < num_depth_files; ++i) {
        depth_files.emplace_back(fmt::format("depth{:06d}.png", i));
    }
    std::cout << "ply_path: " << ply_path << std::endl;
    std::cout << "depth_dir: " << depth_dir << std::endl;
    Eigen::MatrixXd traj_data = LoadEigenMatrixFromTextFile<double>(
        (depth_dir / "traj.csv").string(),
    ...
    try {
        ...
        auto update_render = [&](open3d::visualization::Visualizer *vis) {
            ...
            if (show_depth_rays || show_depth_points || show_surface_samples ||
                show_region_samples || show_along_ray_samples) {
                // r-zyx order
                ...
                auto depth_frame_3d_setting = std::make_shared<DepthFrame3Dd::Setting>();
                int new_height = 0, new_width = 0;
                if (down_sample) {
                    double factor = 1 / down_sample_factor;
                    std::tie(new_height, new_width) =
                        depth_frame_3d_setting->camera_intrinsic.Resize(factor);
                }
                auto depth_frame_3d = std::make_shared<DepthFrame3Dd>(depth_frame_3d_setting);
                if (show_depth_rays || show_depth_points || !surface_samples_ready ||
                    !region_samples_ready || !along_ray_samples_ready) {
                    cv::Mat depth_img = cv::imread(
                        (depth_dir / depth_files[cur_depth_file]).string(),
                        cv::IMREAD_UNCHANGED);
                    if (down_sample) {
                        cv::resize(
                            depth_img,
                    ...
                    std::cout << "depth image size: " << depth.rows() << " x " << depth.cols()
                              << std::endl;
                    auto tic = std::chrono::high_resolution_clock::now();
                    depth_frame_3d->UpdateRanges(rotation, camera_position, depth);
                    // depth isn't scaled yet, do not partition rays
                    auto toc = std::chrono::high_resolution_clock::now();
                    std::cout << "depth_frame_3d->Update takes "
                ...
                if (show_surface_samples && !surface_samples_ready) {
                    ...
                    auto tic = std::chrono::high_resolution_clock::now();
                    depth_frame_3d->SampleNearSurface(
                        10,
                        0.05,
                        1.0,
                        sampled_positions,
                    ...
                    for (long i = 0; i < num_samples; ++i) {
                        ...
                        surface_samples_line_set->points_.emplace_back(
                ...
                if (show_region_samples && !region_samples_ready) {
                    ...
                    for (long i = 0; i < sampled_positions.cols(); ++i) {
                        ...
                        region_samples_line_set->points_.emplace_back(
                            sampled_positions.col(i) +
                            sampled_directions.col(i) * sampled_distances[i]);
                ...
                if (show_along_ray_samples && !along_ray_samples_ready) {
                    ...
                    double max_in_obstacle_dist = 0.05;
                    auto tic = std::chrono::high_resolution_clock::now();
                    depth_frame_3d->SampleAlongRays(
                        range_step,
                        max_in_obstacle_dist,
                        1.0,
                        sampled_positions,
                    ...
                    for (long i = 0; i < num_samples; ++i) {
                        ...
                        along_ray_samples_line_set->points_.emplace_back(
                            sampled_positions.col(i) +
                            sampled_directions.col(i) * sampled_distances[i]);
                        ...
                        along_ray_samples_point_cloud->points_.emplace_back(
                            sampled_positions.col(i));
        ...
        key_to_callback[GLFW_KEY_F7] = [&](open3d::visualization::Visualizer *vis) -> bool {
            if (show_sample_points) {
                ...
                if (show_along_ray_samples) {
                    vis->AddGeometry(along_ray_samples_point_cloud, false);
                }
        ...
        key_to_callback[GLFW_KEY_F8] = [&](open3d::visualization::Visualizer *vis) -> bool {
            ...
            std::cout << "down_sample: " << down_sample << std::endl
                      << "down_sample_factor: " << down_sample_factor << std::endl;
