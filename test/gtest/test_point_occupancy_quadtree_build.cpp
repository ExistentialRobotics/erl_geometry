...
static std::string g_window_name = "PointOccupancyQuadtree_Build";
...
TEST(PointOccupancyQuadtree, Build) {
    ...
    auto tree_setting = std::make_shared<erl::geometry::PointOccupancyQuadtree::Setting>();
    tree_setting->probability_miss = 0.49;  // log_odd_miss = 0
    tree_setting->resolution = g_options.quadtree_resolution;
    auto tree = std::make_shared<erl::geometry::PointOccupancyQuadtree>(tree_setting);
    std::cout << "PointOccupancyQuadtree Setting:" << std::endl << tree->GetSetting() << std::endl;

    // setup visualization
    auto drawer_setting = std::make_shared<erl::geometry::PointOccupancyQuadtree::Drawer::Setting>();
    ...
    std::cout << "PointOccupancyQuadtree::Drawer Setting:" << std::endl << *drawer_setting << std::endl;
    auto drawer = std::make_shared<erl::geometry::PointOccupancyQuadtree::Drawer>(drawer_setting, tree);
    ...
    for (long i = 0; i < max_update_cnt; ++i) {
        ...
        auto it = tree->BeginLeaf();
        auto end = tree->EndLeaf();
        std::size_t num_points = 0;
        for (; it != end; ++it) {
            auto &node_points = it->GetPoints();
            if (node_points.empty()) { continue; }
            num_points += node_points.size();
            for (const auto &point : node_points) {
                int x = grid_map_info->MeterToGridForValue(point[0], 0);
                int y = grid_map_info->Shape(1) - grid_map_info->MeterToGridForValue(point[1], 1);
                cv::circle(img, cv::Point(x, y), 2, cv::Scalar(0, 255, 0, 255), -1);
            }
        }
