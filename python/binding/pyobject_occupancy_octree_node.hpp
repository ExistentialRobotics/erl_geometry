...
namespace erl::geometry {
    class __attribute__((visibility("hidden"))) PyObjectOccupancyOctreeNode
        : public OccupancyOctreeNode {
    protected:
        py::object m_py_object_ = py::none();  // Store the Python object

    public:
        explicit PyObjectOccupancyOctreeNode(
        ...
        operator==(const AbstractOctreeNode &other) const override {
            if (OccupancyOctreeNode::operator==(other)) {
                const auto &other_node =
                    reinterpret_cast<const PyObjectOccupancyOctreeNode &>(other);
