...
BindLidar2D(const py::module &m) {
    ...
    py::class_<Lidar2D::Setting, YamlableBase, std::shared_ptr<Lidar2D::Setting>>(
        py_lidar,
        "Setting")
        .def(py::init<>())
    ...
    py_lidar
        .def(
            py::init<std::shared_ptr<Lidar2D::Setting>, std::shared_ptr<Space2D>>(),
            py::arg("setting").none(false),
            py::arg("space2d").none(false))
        ...
        .def(
            "scan",
            py::overload_cast<double, const Eigen::Ref<const Eigen::Vector2d> &, bool>(
                &Lidar2D::Scan,
                py::const_),
        ...
        .def(
            "scan",
            py::overload_cast<
                const Eigen::Ref<const Eigen::Matrix2d> &,
                const Eigen::Ref<const Eigen::Vector2d> &,
                bool>(&Lidar2D::Scan, py::const_),
            py::arg("rotation"),
            py::arg("translation"),
            py::arg("parallel") = false,
            py::call_guard<py::gil_scoped_release>())
        .def(
            "scan_multi_poses",
            py::overload_cast<const std::vector<Eigen::Matrix3d> &, bool>(
                &Lidar2D::ScanMultiPoses,
                py::const_),  // const method should use py::const_
            // a vector of Eigen::VectorXd is returned.
            // we assume the number of poses is not too large, so the overhead of returning a vector
            // is acceptable.
            py::arg("poses"),
            py::arg("parallel") = false,
            py::call_guard<py::gil_scoped_release>());
