...
BindRangeSensor3DImpl(const py::module &m, const char *name) {
    ...
    py::class_<T, std::shared_ptr<T>>(m, name)
        .def_property_readonly(
            ...
            [](const T &self) -> py::array_t<double> {
                return py::cast_to_array(self.GetRayDirectionsInFrame());
            })
        .def(
            "add_mesh",
            py::overload_cast<const std::string &>(&T::AddMesh, py::const_),
            py::arg("mesh_path"))
        .def(
            "add_mesh",
            py::overload_cast<
                const Eigen::Ref<const Eigen::Matrix3X<Dtype>> &,
                const Eigen::Ref<const Eigen::Matrix3Xi> &>(&T::AddMesh, py::const_),
            py::arg("vertices"),
            py::arg("triangles"))
        .def(
            "add_mesh",
            py::overload_cast<
                const std::vector<Eigen::Vector3<Dtype>> &,
                const std::vector<Eigen::Vector3i> &>(&T::AddMesh, py::const_),
            py::arg("vertices"),
            py::arg("triangles"))
        .def(
            "scan",
        ...
        .def_property_readonly("cached_normals", [](const T &self) -> py::array_t<double> {
            return py::cast_to_array(self.GetCachedNormals());
        });
}
