...
#pragma omp critical
    ...
    public:
        ...
        [[nodiscard]] QuadtreeKey
        CoordToKey(const Eigen::Ref<const Eigen::Vector2d> &coord, uint32_t depth) const {
        ...
        CoordToKeyChecked(double coordinate, QuadtreeKey::KeyType &key) const {
            const int scaled_coord = std::floor(coordinate * m_resolution_inv_) + m_tree_key_offset_;
            if ((scaled_coord >= 0) && static_cast<uint32_t>(scaled_coord) < (m_tree_key_offset_ << 1)) {
                key = scaled_coord;
                return true;
            }
            return false;
        ...
        CoordToKeyChecked(const Eigen::Ref<const Eigen::Vector2d> &coord, uint32_t depth, QuadtreeKey &key) const {
