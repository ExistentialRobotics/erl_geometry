...
namespace erl::geometry {
    ...
    public:
        ...
        explicit QuadtreeImpl(const std::shared_ptr<InterfaceSetting> &setting)
            : Interface(setting) {
            QuadtreeImpl::ApplySetting();
        }

        QuadtreeImpl(const QuadtreeImpl &) = delete;  // no copy constructor
...
#pragma omp critical
        ...
        [[maybe_unused]] [[nodiscard]] inline std::size_t
    ...
    public:
        ...
        ComputeCommonAncestorKey(const QuadtreeKey &key1, const QuadtreeKey &key2, QuadtreeKey &ancestor_key, uint32_t &ancestor_depth) const {
            ...
            QuadtreeKey::KeyType ancestor_mask = ((1 << kTreeDepth) - 1) << level;
        ...
        ComputeWestNeighborKey(const QuadtreeKey &key, uint32_t depth, QuadtreeKey &neighbor_key) const {
            QuadtreeKey::KeyType offset = 1 << (this->m_setting_->tree_depth - depth);
        ...
        ComputeEastNeighborKey(const QuadtreeKey &key, uint32_t depth, QuadtreeKey &neighbor_key) const {
            const uint32_t &kTreeDepth = this->m_setting_->tree_depth;
            QuadtreeKey::KeyType offset = 1 << (kTreeDepth - depth);
        ...
        public:
            ...
            [[nodiscard]] inline Aabb2D
            GetNodeAabb() const {
                Eigen::Vector2d center(this->GetX(), this->GetY());
                return {center, this->GetNodeSize() * 0.5};
            }

            [[maybe_unused]] [[nodiscard]] inline const QuadtreeKey &
            GetKey() const {
                return m_stack_.back().key;
            }

            [[maybe_unused]] [[nodiscard]] inline QuadtreeKey
            GetIndexKey() const {
                const StackElement &kTop = m_stack_.back();
        ...
        public:
            ...
            InAabbIteratorBase(
                ...
                if (this->m_tree_->CoordToKeyChecked(aabb_min_x, aabb_min_y, m_aabb_min_key_) &&
                    ...
                    QuadtreeKey::KeyType center_offset_key = this->m_tree_->m_tree_key_offset_ >> top.node->GetDepth();
         ...
         */
        [[nodiscard]] bool
        ComputeRayCoords(double sx, double sy, double ex, double ey, std::vector<Eigen::Vector2d> &ray) const {
            ray.clear();
         ...
         * @param x
         * @param y
         * @param delete_depth delete_depth to start deleting nodes, nodes at delete_depth >= delete_depth will be deleted. If delete_depth == 0, delete at the
         * lowest level.
         * @return
         */
        inline uint32_t
        DeleteNode(double x, double y, uint32_t delete_depth = 0) {
            QuadtreeKey key;
    ...
    protected:
         ...
         */
        inline virtual void
        OnDeleteNodeChild(Node * /*node*/, Node * /*child*/, const geometry::QuadtreeKey & /*key*/) {}
    ...
    public:
         ...
         */
        [[nodiscard]] inline const Node *
        Search(double x, double y, uint32_t max_depth = 0) const {
            QuadtreeKey key;
        ...
        Search(const QuadtreeKey &key, uint32_t max_depth = 0) const {
            const uint32_t &kTreeDepth = this->m_setting_->tree_depth;
            ERL_DEBUG_ASSERT(max_depth <= kTreeDepth, "Depth must be in [0, %u], but got %u.", kTreeDepth, max_depth);

            if (m_root_ == nullptr) { return nullptr; }
            if (max_depth == 0) { max_depth = kTreeDepth; }

            // generate appropriate key for the given max_depth
            QuadtreeKey key_at_depth = key;
            if (max_depth != kTreeDepth) { key_at_depth = AdjustKeyToDepth(key_at_depth, max_depth); }

            // search
            const Node *node = m_root_.get();
            int min_level = kTreeDepth - max_depth;
            // follow nodes down to the requested level (for level = 0, it is the leaf level)
            for (int level = kTreeDepth - 1; level >= min_level; --level) {
