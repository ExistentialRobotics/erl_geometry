...
namespace erl::geometry {
    ...
    struct OccupancyNdTreeSetting : public common::OverrideYamlable<NdTreeSetting, OccupancyNdTreeSetting> {
        float log_odd_min = -2;           // minimum log-odd value, =0.12 in probability
        float log_odd_max = 3.5;          // maximum log-odd value, = 0.97 in probability
        float log_odd_hit = 0.85;         // log-odd value to add when a cell is hit by a ray, =0.7 in probability
        float log_odd_miss = -0.4;        // log-odd value to add when a cell is gone through by a ray, =0.4 in probability
        float log_odd_occ_threshold = 0;  // threshold that is used to decide whether a cell is occupied or not, =0.5 in probability

        inline void
        SetProbabilityHit(double p) {
            log_odd_hit = logodd::LogOdd(p);
            ERL_WARN_COND(log_odd_hit <= 0, "ProbabilityHit should be > 0, but is %f", log_odd_hit);
        }
        ...
        SetProbabilityMiss(double p) {
            ...
            ERL_WARN_COND(log_odd_miss >= 0, "ProbabilityMiss should be < 0, but is %f", log_odd_miss);
        }
        ...
        inline void
        SetProbabilityOccupiedThreshold(double p) {
        ...
        operator==(const NdTreeSetting& rhs) const override {
            if (NdTreeSetting::operator==(rhs)) {
                auto that = reinterpret_cast<const OccupancyNdTreeSetting&>(rhs);
...
namespace YAML {
    template<>
    struct convert<erl::geometry::OccupancyNdTreeSetting> {
        static Node
