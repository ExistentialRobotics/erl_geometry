...
namespace erl::geometry::sdf_util {
    ...
    constexpr int MULTITHREAD_MIN_ITEMS = 50;
...
namespace nanoflann {
    ...
    struct KDTreeEigenRefAdaptor {
        ...
        explicit KDTreeEigenRefAdaptor(
            Eigen::Ref<const MatrixType> mat,
            bool build = true,
        ...
        // Rebuild the KD tree from scratch. Call if the data is updated.
        void
        rebuild() {
            if (index) delete index;
            index =
                new index_t(DIM, *this, nanoflann::KDTreeSingleIndexAdaptorParams(leaf_max_size));
            index->buildIndex();
        }
