...
namespace erl::geometry {
    ...
    AbstractOccupancyQuadtree::WriteBinary(const std::string &filename) {
        ERL_DEBUG("Writing Quadtree to file: %s", std::filesystem::absolute(filename).c_str());
        ...
        file.close();
        if (success) {
            ERL_DEBUG("Successfully wrote Quadtree of type %s, size %zu", GetTreeType().c_str(), GetSize());
        } else {
            ERL_WARN("Failed to write Quadtree of type %s, size %zu", GetTreeType().c_str(), GetSize());
        }
        return success;
    }
