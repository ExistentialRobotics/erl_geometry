...
namespace erl::geometry {
    ...
    Space2D::Space2D(
        ...
        ERL_ASSERTM(
            static_cast<ssize_t>(ordered_object_normals.size()) == n_obj,
            "#groups_of_normals != #objects.");
    ...
    Space2D::Space2D(
        ...
        ERL_ASSERTM(
            outside_flags.size() == n_obj,
            "#outside_flags({}) != #objects({}).",
            outside_flags.size(),
            n_obj);
        ...
        m_kdtree_ = std::make_unique<KdTree>(2, m_surface_->vertices);
        // don't use kd-tree because vertices are not always dense
        ComputeNormals(delta, false, parallel);
    }
    ...
    Space2D::Space2D(
        ...
        // convert to meters
        m_surface_->vertices =
            grid_map_info.PixelToMeterForPoints(m_surface_->vertices.array().round().cast<int>());
        delta = grid_map_info.Resolution().minCoeff();
        m_kdtree_ = std::make_unique<KdTree>(2, m_surface_->vertices);
        // use kd-tree because vertices generated by marching square are dense
        ComputeNormals(delta, true, parallel);
    }

    Space2D
    Space2D::AddObstacles(
        ...
        for (long i = 0; i < num_objects; ++i) {
            ordered_object_vertices.emplace_back(
                m_surface_->GetObjectVertices(static_cast<int>(i)));
    ...
    Eigen::MatrixX8U
    Space2D::GenerateMapImage(const common::GridMapInfo2Dd &grid_map_info, const bool anti_aliased)
        const {
        cv::Mat cv_map_image =
            cv::Mat::zeros(grid_map_info.Height(), grid_map_info.Width(), CV_8UC1);
        const long n_obj = m_surface_->GetNumObjects();
        for (int i = 0; i < n_obj; ++i) {
            std::vector<std::vector<cv::Point>> contours(1);
            Eigen::Matrix2Xi vertices =
                grid_map_info.MeterToPixelForPoints(m_surface_->GetObjectVertices(i));
            const long n_vertices = vertices.cols();
            for (int j = 0; j < n_vertices; ++j) {
                contours[0].emplace_back(vertices(0, j), vertices(1, j));
            }
            cv::drawContours(
                cv_map_image,
                contours,
                0,
                m_surface_->outside_flags[i] ? 0 : 255,
                -1,
                anti_aliased ? cv::LINE_8 : cv::LINE_AA);
        }
        // map image: x-axis to right, y-axis to top
        Eigen::MatrixX8U map_image(grid_map_info.Height(), grid_map_info.Width());
    ...
    Space2D::ComputeSdfImage(
        ...
        // y up, x right, column-major
        Eigen::MatrixXd sdf(grid_map_info.Height(), grid_map_info.Width());
...
#pragma omp parallel for if (parallel) default(none) \
    ...
    /**
     * use kd-tree to find the nearest vertex to speed up the computation: this works well ONLY when
     * the surface points are dense enough.
     * @param q
     * @param sign_method
     * @return
     */
    ...
    Space2D::ComputeSdfWithKdtree(const Eigen::Vector2d &q, const SignMethod sign_method) const {
        ...
        const auto [idx_neighbor1, idx_neighbor2] =
            m_surface_->GetVertexNeighbors(static_cast<int>(idx_vertex_0));
        ...
        const double dist_1 = ComputeNearestDistanceFromPointToLineSegment2D(
            q.x(),
            q.y(),
            v0.x(),
            v0.y(),
            v1.x(),
            v1.y());
        const double dist_2 = ComputeNearestDistanceFromPointToLineSegment2D(
            q.x(),
            q.y(),
            v0.x(),
            v0.y(),
        ...
        if (IsNegativeSdf(
                sign_method,
                q,
                static_cast<int>(idx_vertex_0),
                static_cast<int>(idx_vertex_1))) {
            sdf = -sdf;
        }
    ...
    Space2D::ComputeSdfGreedily(
        ...
        for (int j = 1; j < m_surface_->GetNumLines(); ++j) {
            ...
            if (const double dist = ComputeNearestDistanceFromPointToLineSegment2D(
                    q.x(),
                    q.y(),
                    v0.x(),
                    v0.y(),
    ...
    shared(grid_map_info, query_directions, out, Eigen::Dynamic)
        for (int v = 0; v < grid_map_info.Height(); ++v) {
            for (int u = 0; u < grid_map_info.Width(); ++u) {
                ...
                out(v, u) = ComputeDdf(
                    point.replicate(1, query_directions.cols()),
                    query_directions,
                    false);
            }
        }
...
#pragma omp parallel for if (parallel) default(none) \
    shared(m_surface_, query_points, query_directions, ddf)
        for (int i = 0; i < query_points.cols(); ++i) {
            ...
            for (int j = 0; j < n_lines; ++j) {
                ComputeIntersectionBetweenRayAndLine2D<double>(
                    ...
                    intersected);
                // positive ddf only
                if (intersected && lam <= 1. && lam >= 0. && t >= 0. && t < ddf[i]) { ddf[i] = t; }
            }
        }
...
#pragma omp parallel for if (parallel) default(none) \
    shared(n_directions, height, width, grid_map_info, query_directions, out)
        for (int v = 0; v < height; ++v) {
            for (int u = 0; u < width; ++u) {
                Eigen::Vector2d point = grid_map_info.PixelToMeterForPoints(Eigen::Vector2i(u, v));
                out(v, u) =
                    ComputeSddfV1(point.replicate(1, n_directions), query_directions, false);
            }
        }
        ...
        for (int i = 0; i < query_points.cols(); ++i) {
            ...
            for (int j = 0; j < n_lines; ++j) {
                ...
                ComputeIntersectionBetweenRayAndLine2D<double>(
                    ...
                    t,
                    intersected);
                // min_t(p + t * v == surface_point)
                if (intersected && lam <= 1. && lam >= 0. && t < ddf[i]) { ddf[i] = t; }
            }
        }
...
#pragma omp parallel for if (parallel) default(none) \
    shared(n_directions, height, width, grid_map_info, query_directions, sign_method, out)
        for (int v = 0; v < height; ++v) {
            for (int u = 0; u < width; ++u) {
                Eigen::Vector2d point = grid_map_info.PixelToMeterForPoints(Eigen::Vector2i(u, v));
                out(v, u) = ComputeSddfV2(
                    point.replicate(1, n_directions),
                    query_directions,
                    sign_method,
                    false);
            }
...
#pragma omp parallel for if (parallel) default(none) \
        ...
        for (int i = 0; i < query_points.cols(); ++i) {
            ...
            for (int k = 0; k < lams.size(); ++k) {
                ...
                if (lam <= 1. && lam >= 0.) {
                    if (is_negative) {
                        if (t <= 0.) {
                            if (!sddf_opt.has_value() || -t < std::abs(sddf_opt.value())) {
                                sddf_opt = t;
                            }
                        }
                    } else {
    ...
    bool
    Space2D::IsNegativeSdf(
        SignMethod sign_method,
        const Eigen::Ref<const Eigen::Vector2d> &q,
        const int vertex_0_idx,
        const int vertex_1_idx) const {
        switch (sign_method) {
            ...
            case SignMethod::kPolygon:
                ERL_ASSERTM(
                    ...
                    "outside flags of the polygons are not available. SignMethod::kPolygon "
                    "cannot be used currently.");
                return IsNegativeSdfByPolygon(q, vertex_0_idx);
        }

        throw std::runtime_error(
            "Unknown sign method: " + std::to_string(static_cast<int>(sign_method)));
    }

    bool
    Space2D::IsNegativeSdfByPointNormal(
        const Eigen::Ref<const Eigen::Vector2d> &q,
        const int vertex_0_idx) const {
        return m_surface_->normals.col(vertex_0_idx)
                   .dot(q - m_surface_->vertices.col(vertex_0_idx)) < 0.;
    }

    bool
    Space2D::IsNegativeSdfBySegmentNormal(
        const Eigen::Ref<const Eigen::Vector2d> &q,
        const int vertex_0_idx,
        const int vertex_1_idx) const {
        Eigen::Vector2d line_vec =
            m_surface_->vertices.col(vertex_1_idx) - m_surface_->vertices.col(vertex_0_idx);
        line_vec.normalize();
        const Eigen::Vector2d normal =
            m_surface_->normals.col(vertex_0_idx) -
            line_vec * m_surface_->normals.col(vertex_0_idx).dot(line_vec);
        return normal.dot(q - m_surface_->vertices.col(vertex_0_idx)) < 0.;
    }

    bool
    Space2D::IsNegativeSdfByPolygon(
        const Eigen::Ref<const Eigen::Vector2d> &q,
        const int vertex_0_idx) const {
    ...
    Space2D::ComputeNormals(const double delta, const bool use_kdtree, const bool parallel) const {
        ...
        // clang-format off
            deltas << -delta, 0, +delta, 0,
                      0, -delta, 0, +delta;
        // clang-format on
        for (int i = 0; i < static_cast<int>(n_vtx); ++i) {
            query_points.block<2, 4>(0, i * 4) << deltas.colwise() + m_surface_->vertices.col(i);
        }
#if EIGEN_VERSION_AT_LEAST(3, 4, 0)
        Eigen::Matrix4Xd sdf =
            ComputeSdf(query_points, SignMethod::kPolygon, use_kdtree, parallel).reshaped(4, n_vtx);
#else
        Eigen::VectorXd sdf_vec =
            ComputeSdf(query_points, SignMethod::kPolygon, use_kdtree, parallel);
        Eigen::Matrix4Xd sdf = Eigen::Map<Eigen::Matrix4Xd>(sdf_vec.data(), 4, n_vtx);
#endif
        m_surface_->normals =
            (sdf.block(2, 0, 2, n_vtx) - sdf.block(0, 0, 2, n_vtx)) / (delta * 2.);
    ...
    Space2D::ComputeOutsideFlags(
        ...
        const double free_threshold) const {
        const long n_obj = m_surface_->GetNumObjects();
    ...
    Space2D::ComputeOutsideFlags() const {
        ...
        for (int i = 0; i < n_obj; ++i) {
            ...
            bool &&is_negative_sdf =
                (m_surface_->GetObjectNormals(i).array() * v.array()).mean() < 0.;
