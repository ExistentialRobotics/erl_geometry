...
namespace erl::geometry {
    PointOccupancyOctreeNode::PointOccupancyOctreeNode(const PointOccupancyOctreeNode &other)  // NOLINT(*-copy-constructor-init, *-no-recursion)
        : OccupancyOctreeNode(other.m_value_),                                                 // should not call the copy constructor of OccupancyOctreeNode
          m_points_(other.m_points_) {
        CopyChildren<PointOccupancyOctreeNode>(other);
    }

    std::istream &
    PointOccupancyOctreeNode::ReadData(std::istream &s) {
        ...
        for (auto &point: m_points_) {
            ...
            s.read(reinterpret_cast<char *>(&point[2]), sizeof(double));
        }
        return s;
    }

    std::ostream &
    PointOccupancyOctreeNode::WriteData(std::ostream &s) const {
        ...
        for (const auto &point: m_points_) {
            ...
            s.write(reinterpret_cast<const char *>(&point[2]), sizeof(double));
        }
        return s;
    }

    PointOccupancyOctree::PointOccupancyOctree(double resolution)
        : OccupancyOctreeBase<PointOccupancyOctreeNode>(resolution) {
        s_init_.EnsureLinking();
    }

    PointOccupancyOctree::PointOccupancyOctree(const std::shared_ptr<Setting> &setting)
        : OccupancyOctreeBase<PointOccupancyOctreeNode>(setting) {
        m_max_num_points_per_node_ = setting->max_num_points_per_node;
    }

    PointOccupancyOctree::PointOccupancyOctree(const std::string &filename)
        : OccupancyOctreeBase<PointOccupancyOctreeNode>(0.1) {  // resolution will be set by readBinary
        ERL_ASSERTM(this->ReadBinary(filename), "Failed to read %s from file: %s", GetTreeType().c_str(), filename.c_str());
    }

    std::shared_ptr<PointOccupancyOctree::Setting>
    PointOccupancyOctree::GetSetting() const {
    ...
    bool
    PointOccupancyOctree::IsNodeCollapsible(const erl::geometry::PointOccupancyOctreeNode *node) const {
    ...
    void
    PointOccupancyOctree::InsertPointCloud(
        ...
        for (auto &[key, point_indices]: end_point_mapping) {
            ...
            PointOccupancyOctreeNode *node = this->Search(key, depth);
    ...
    void
    PointOccupancyOctree::InsertPointCloudRays(
        const Eigen::Ref<const Eigen::Matrix3Xd> &points,
        const Eigen::Ref<const Eigen::Vector3d> &sensor_origin,
        double max_range,
        bool parallel,
    ...
    bool
    PointOccupancyOctree::InsertRay(double sx, double sy, double sz, double ex, double ey, double ez, double max_range, bool lazy_eval) {
        double dx = ex - sx;
        double dy = ey - sy;
        double dz = ez - sz;
        ...
        if (this->CoordToKeyChecked(ex, ey, ez, key)) {
            ...
            if (node != nullptr && this->IsNodeOccupied(node)) {
                ERL_DEBUG_ASSERT(depth == this->mk_TreeDepth_, "depth is wrong for an occupied node.");
                if (node->GetNumPoints() < m_max_num_points_per_node_) { node->AddPoint(Eigen::Vector3d(ex, ey, ez)); }
            }
        }
        return true;
    }
