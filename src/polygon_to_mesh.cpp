...
namespace erl::geometry {
    ...
    PolygonToMesh(const std::vector<std::vector<Eigen::Vector2d>> &polygon, double z, bool upward_normal) {
        auto triangles_vertex_indices = earcut(polygon);
        ...
        for (const auto &polygon_i: polygon) {
            for (const auto &vertex: polygon_i) { mesh->vertices_.emplace_back(vertex[0], vertex[1], z); }
        }
        for (std::size_t i = 0; i < triangles_vertex_indices.size(); i += 3) {
            mesh->triangles_.emplace_back(triangles_vertex_indices[i], triangles_vertex_indices[i + 1], triangles_vertex_indices[i + 2]);
        }
