...
namespace erl::geometry {
    ...
    RangeSensor3D<Dtype>::Scan(
        ...
        // will be copied to tensor later
        const Eigen::Vector3f &ray_start = translation.template cast<float>();
        // column major
        open3d::core::Tensor rays({n_rows, n_cols, 6}, open3d::core::Dtype::Float32);
        auto *rays_ptr = rays.GetDataPtr<float>();
#pragma omp parallel for default(none) \
    shared(n_rows, n_cols, orientation, directions, scales, ray_start, rays_ptr)
        for (long v = 0; v < n_cols; ++v) {
            ...
            for (long u = 0; u < n_rows; ++u) {
                ...
                Eigen::Vector3f direction =
                    (orientation * directions.data()[base1]).template cast<float>();
        ...
        // cast rays
        const std::unordered_map<std::string, open3d::core::Tensor> cast_results =
            m_scene_->CastRays(rays);
        const std::vector<float> ranges = cast_results.at("t_hit").ToFlatVector<float>();
        const std::vector<uint32_t> geometry_ids =
            cast_results.at("geometry_ids").ToFlatVector<uint32_t>();
        const std::vector<float> normals =
            cache_normals ? cast_results.at("primitive_normals").ToFlatVector<float>()
                          : std::vector<float>{};
        const uint32_t invalid_id = open3d::t::geometry::RaycastingScene::INVALID_ID();

        // copy results to Eigen matrix
        if (cache_normals && (m_normals_.rows() != n_rows || m_normals_.cols() != n_cols)) {
            m_normals_ = Eigen::MatrixX<Vector3>(n_rows, n_cols);
        }
