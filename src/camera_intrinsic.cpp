...
namespace erl::geometry {
    ...
    void
    CameraIntrinsic::ComputeFrameDirections(Eigen::MatrixX<Eigen::Vector3d> &dirs) const {
        ...
        const double fx_inv = 1.0 / camera_fx;
        const double fy_inv = 1.0 / camera_fy;

#pragma omp parallel for default(none) shared(fx_inv, fy_inv, dirs, Eigen::Dynamic)
        for (long u = 0; u < image_width; ++u) {
            const double xu = (static_cast<double>(u) - camera_cx) * fx_inv;
            for (long v = 0; v < image_height; ++v) {
                const double yv = (static_cast<double>(v) - camera_cy) * fy_inv;
                // normalized image coordinates
                // v <--> image coordinate y, u <--> image coordinate x
                Eigen::Vector3d &dir_frame = dirs(v, u);
                dir_frame << xu, yv, 1.0;
                dir_frame.normalize();
            }
        }
    ...
    void
    CameraIntrinsic::ComputeFrameDirections(Eigen::MatrixX<Eigen::Vector2d> &coords, Eigen::MatrixX<Eigen::Vector3d> &dirs) const {
...
#pragma omp parallel for default(none) shared(fx_inv, fy_inv, coords, dirs, Eigen::Dynamic)
    ...
    void
    CameraIntrinsic::ConvertDepthToDistance(const Eigen::MatrixXd &depth, Eigen::MatrixXd &distance) const {
...
#pragma omp parallel for default(none) shared(fx_inv, fy_inv, depth, distance, Eigen::Dynamic)
        for (long u = 0; u < image_width; ++u) {
            const double xu = (static_cast<double>(u) - camera_cx) * fx_inv;
            double *distance_ptr = distance.col(u).data();
            const double *depth_ptr = depth.col(u).data();
            for (long v = 0; v < image_height; ++v) {
                const double yv = (static_cast<double>(v) - camera_cy) * fy_inv;
    ...
    void
    CameraIntrinsic::ConvertDistanceToDepth(const Eigen::MatrixXd &distance, Eigen::MatrixXd &depth) const {
...
#pragma omp parallel for default(none) shared(fx_inv, fy_inv, depth, distance, Eigen::Dynamic)
        for (long u = 0; u < image_width; ++u) {
            const double xu = (static_cast<double>(u) - camera_cx) * fx_inv;
            double *depth_ptr = depth.col(u).data();
            const double *distance_ptr = distance.col(u).data();
            for (long v = 0; v < image_height; ++v) {
                const double yv = (static_cast<double>(v) - camera_cy) * fy_inv;
    ...
    void
    CameraIntrinsic::ConvertRgbdToPointCloud(
        ...
        if (has_optical_pose) {
            rotation = optical_pose->topLeftCorner<3, 3>();
            translation = optical_pose->topRightCorner<3, 1>();
        }
...
#pragma omp parallel for default(none) shared(fx_inv, fy_inv, depth, rgb, has_optical_pose, rotation, translation, points, colors, Eigen::Dynamic)
        for (long u = 0; u < image_width; ++u) {
            ...
            for (long v = 0, i = u * image_height; v < image_height; ++v, ++i) {
                ...
                if (!rgb.empty()) {
                    ...
                    Eigen::Vector3d &color_point = colors[i];
                    color_point[0] = static_cast<double>(color[0]) / 255.0;
                    color_point[1] = static_cast<double>(color[1]) / 255.0;
                    color_point[2] = static_cast<double>(color[2]) / 255.0;
                }
            }
        }
        ...
        for (std::size_t i = 0; i < points.size(); ++i) {
            const double &d = depth.data()[i];
            if (std::isfinite(d) && d > 0) { continue; }
            std::size_t j = points.size() - 1;
            while (j > i) {
                if (const double &d2 = depth.data()[j]; std::isfinite(d2) && d2 > 0) { break; }
                --j;
            }
            points[i] = points[j];
            points.pop_back();
