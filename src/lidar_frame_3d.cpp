...
#pragma omp parallel for collapse(2) default(none) shared(num_azimuths, num_elevations, Eigen::Dynamic)
        for (long elevation_idx = 0; elevation_idx < num_elevations; ++elevation_idx) {
            for (long azimuth_idx = 0; azimuth_idx < num_azimuths; ++azimuth_idx) {
                ...
                const double azimuth = m_azimuth_frame_(azimuth_idx, elevation_idx);
                const double elevation = m_elevation_frame_(azimuth_idx, elevation_idx);
    ...
    LidarFrame3D::ComputeClosestEndPoint(
        ...
        if (brute_force) {
            ...
            const long n_azimuths = GetNumAzimuthLines();
            const long n_elevations = GetNumElevationLines();
            for (long azimuth_idx = 0; azimuth_idx < n_azimuths; ++azimuth_idx) {
                for (long elevation_idx = 0; elevation_idx < n_elevations; ++elevation_idx) {
                    if (const double d = (m_end_pts_world_(azimuth_idx, elevation_idx) - position_world).squaredNorm(); d < distance) {
    ...
    LidarFrame3D::SampleAlongRays(
        ...
        for (const long &ray_idx: ray_indices) {
            const long azimuth_idx = m_hit_ray_indices_(0, ray_idx);
            ...
            for (long elevation_idx = 0; elevation_idx < num_elevations; ++elevation_idx) {
                ...
                if (azimuth_idx == 0) {
                    ...
                    const double range_diff = std::abs(  //
                        (range - m_ranges_(azimuth_idx, elevation_idx - 1)) / (elevation - m_elevation_frame_(azimuth_idx, elevation_idx - 1)));
                    ...
                    double range_diff = std::abs(  //
                        (range - m_ranges_(azimuth_idx - 1, elevation_idx)) / (azimuth - m_azimuth_frame_(azimuth_idx - 1, elevation_idx)));
    ...
    LidarFrame3D::SampleInRegionHprThread(
        ...
        for (long position_idx = 0; position_idx < num_positions; ++position_idx) {
            ...
            const long &hit_azimuth_index = m_hit_ray_indices_(0, hit_index);
            const long &hit_elevation_index = m_hit_ray_indices_(1, hit_index);
    ...
    LidarFrame3D::SampleInRegionVrsThread(
        ...
        for (const long *hit_point_index_ptr = hit_point_index_start; hit_point_index_ptr < hit_point_index_end; ++hit_point_index_ptr) {
            ...
            const long hit_azimuth_index = m_hit_ray_indices_(0, *hit_point_index_ptr);
            const long hit_elevation_index = m_hit_ray_indices_(1, *hit_point_index_ptr);
